...
struct FeedsFeature {
    ...
    var body: some ReducerOf<FeedsFeature> {
        Reduce { state, action in
            ...
            case .fetchGifList:
                ...
                return .run { send in
                    do {
                        ...
                        await send(.feedsResponse(.failure(error)))
                    }
                }
            case .feedsResponse(.success(let gifList)):
                ...
                state.gifList = IdentifiedArray(
                    uniqueElements: gifList.gifs.enumerated().map { index, gif in
                        GifFeature.State(
                            id: UUID(),
                            gif: gif,
                            user: userMap[gif.userName]!,
                            pageIndex: index
                        )
                    }
                )
        ...
        .forEach(\.gifList, action: \.gifAction) {
            GifFeature()
        }
    }
}
